...
object ModpackService {
    ...
    fun Modpack.Version.startInstall(modpackName: String?=null) {
        TaskFragment("下载整合包 ${modpackName?:""} ${totalSize?.humanFileSize} ") {
            ModpackService.installVersion(modpackId, name, mods) { progress ->
                this.log(progress)
            }
        }.go()
    }
    ...
    fun Host.startPlay() = ioTask {
        val status = server.makeRequest<HostStatus>("host/${_id}/status").data ?: let {
            alertErr("获取房间状态失败，无法游玩")
            return@ioTask
        }
        if(!isVersionInstalled(modpackId, packVer)){
            confirm("未下载此房间的整合包，无法游玩。要现在开始下载吗？"){
                //todo dl version
                server.makeRequest<Modpack.Version>()
                GameService.startInstallModpackVersion(modpackId, packVer)
            }
            return@ioTask
        }
